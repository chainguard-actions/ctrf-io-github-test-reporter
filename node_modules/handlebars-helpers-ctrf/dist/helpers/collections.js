 ...
 */
export const isEmptyHelper = {
    name: "isEmpty",
    category: "Collections",
    fn: (collection, options) => {
        let empty = false;
        if (collection == null) {
            empty = true;
        }
...
export const iterateHelper = {
    ...
    fn: (collection, options) => {
        ...
        const fn = options
            .fn;
        const inverse = typeof options.inverse === "function"
            ? options.inverse
            : undefined;
        let output = "";
        if (Array.isArray(collection)) {
            if (collection.length === 0 && inverse)
                return inverse();
            ...
            if (keys.length === 0 && inverse)
                return inverse();
            keys.forEach((key) => {
                // Expose key as @key in Handlebars
                output += fn(collection[key], {
                    data: { key },
                });
            });
            return output;
...
export const collectionsHelpers = [isEmptyHelper, iterateHelper];
